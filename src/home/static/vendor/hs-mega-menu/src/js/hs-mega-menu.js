...
export default class HSMegaMenu {
  ...
  _init() {
    ...
    for (let i = 0; i < that.collection.length; i += 1) {
      ...
      const getItemSettings = ($el) => {
        ...
        itemSettings = { ...itemSettings, ...dataSettings };
      ...
      document.addEventListener("keyup", (e) => {
        if (!e.target.closest(`${_options.classMap.hasMegaMenu}, ${_options.classMap.hasSubMenu}`) || e.target.closest("input")) {
          return false
        }
  ...
  MegaMenuItem(el, menu, params) {
    ...
    // Listeners
    const myDesktopCSSAnimationEnable = desktopCSSAnimationEnable($menu, itemSettings),
