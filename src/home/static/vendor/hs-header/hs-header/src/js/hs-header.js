...
export default class HSHeader {
  ...
  init() {
    const self = this,
      { element } = this
    const dataSettings = element.hasAttribute("data-hs-header-options") ? JSON.parse(element.getAttribute("data-hs-header-options")) : {}

    if (!element || element.hasAttribute("HSHeader")) {
      return
    }

    this.config = { ...this.defaults, ...dataSettings, ...this.config }
    ...
    function onResize() {
      ...
      self.resizeTimeOutId = setTimeout(() => {
        // self.checkViewport()
        self.update()
      }, 100)
    }
  ...
  _detectObservers() {
    ...
    if (this.element.classList.contains("navbar-sticky-top")) {
      if (this.element.classList.contains("navbar-show-hide")) {
        observers.xs.push(new HSHeaderMomentShowHideObserver(this.element).init())
      } else if (this.element.classList.contains("navbar-toggle")) {
        observers.xs.push(new HSHeaderHideSectionObserver(this.element).init())
      }
    ...
    if (this.element.classList.contains("navbar-invulnerable")) {
      observers.xs.push(new HSHeaderWithoutBehaviorObserver(this.element).init())
    }

    // Abs top & Static
    if (this.element.classList.contains("navbar-absolute-top") || this.element.classList.contains("navbar-static")) {
      if (this.element.classList.contains("navbar-show-hide")) {
        observers.xs.push(new HSHeaderShowHideObserver(this.element, this.config).init())
      }
    }
  ...
  fixMediaDifference(element) {
    if (!element || !element.length || !element.filter((el) => el.closest('[class*="navbar-side"]')).length) {
      return
    }
  ...
  update() {
    for (const viewport in this.observers) {
