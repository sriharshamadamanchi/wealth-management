...
import { Show, children, createEffect, createMemo, createSignal, onCleanup, onMount, splitProps } from "solid-js";
...
const Swiper = (props) => {
  ...
  const attachEvents = () => {
    if (eventsAssigned || !params().events || !swiperRef) {
      return;
    }
  ...
  onCleanup(() => {
    if (swiperRef) {
      swiperRef.off("_beforeBreakpoint", onBeforeBreakpoint);
    }
