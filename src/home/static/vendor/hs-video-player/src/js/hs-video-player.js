...
export default class HSVideoPlayer {
  ...
  _youTubePlayer($el, settings) {
    const optionsWithSpecificAPI = { videoId: settings.videoId,
      playerVars: {
  ...
  _vimeoPlayer($el, settings) {
    const optionsWithSpecificAPI = { id: settings.videoId,
      autoplay: settings.isAutoplay === true ? 1 : 0,
      muted: settings.isMuted === true ? 1 : 0, ...settings.specificAPIOptions }

    const vimeoIframe = document.getElementById(settings.targetSelector.slice(1, settings.targetSelector.length)),
  ...
  _html5Player($el, settings) {
    const $html5Iframe = document.querySelector(settings.targetSelector)
