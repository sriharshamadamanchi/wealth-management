...
HSCore.components.HSList = {
  ...
  initializeHover ($el, settings, newList) {
    const that = this
      ...
      if (e.which === 40) {
        ...
        if (!activeItem) {
          ...
          if (activeItem.previousElementSibling) {
            ...
            if (newActive.getBoundingClientRect().top < 0) {
  ...
  searchMenu ($el, settings, newList) {
    const that = this
  ...
  searchMenuShow ($el, settings, newList) {
    const that = this
  ...
  searchMenuHide ($el, settings, newList) {
    const that = this
    var searchFiled = $el.querySelector(`.${newList.searchClass}`)
  ...
  emptyBlock ($el, settings, newList) {
    const that = this
